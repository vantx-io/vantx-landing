...
function makeMockUsers() {
  return [
    { id: 'user-engineer', email: 'engineer@vantx.io', full_name: 'Jane Engineer' },
      ...
      selectChain.in = vi.fn((...inArgs: any[]) => {
        ...
        return {
          then(onFulfilled: any, onRejected: any) {
            return Promise.resolve(getNext(table, 'list')).then(onFulfilled, onRejected)
          },
...
describe('notifyTaskEvent — task_updated', () => {
  ...
  it('calls sendEmail only once when assigned_to === created_by (dedup per D-11)', async () => {
    const task = makeMockTask({ assigned_to: 'user-admin', created_by: 'user-admin' })
    const client = makeMockClient()
    const users = [{ id: 'user-admin', email: 'admin@vantx.io', full_name: 'Bob Admin' }]
...
describe('notifyTaskEvent — task_created', () => {
  ...
  it('fetches admin/engineer users and sends email to each (per D-12)', async () => {
    ...
    await notifyTaskEvent('task-001', 'task_created', supabase as any, 'user-admin')
